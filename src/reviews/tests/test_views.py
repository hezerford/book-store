...
def test_create_review_authenticated(authenticated_client, existing_user, create_book):
...
@pytest.mark.django_db
def test_create_review_unauthenticated(client, create_book):
...
@pytest.mark.django_db
def test_delete_review_by_author(authenticated_client, existing_user, create_book):
    book = create_book(slug="book-for-delete")
    review = Review.objects.create(book=book, user=existing_user, rating=4, text="x")
...
def test_delete_review_by_non_author_denied(
    authenticated_client, existing_user, create_book
):
    ...
    assert response.status_code == 302
    assert Review.objects.filter(id=review.id).exists(), "Non-author should not delete"
