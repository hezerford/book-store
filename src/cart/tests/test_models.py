...
class TestCartFunctionality:
    def test_cart_clear(self, create_cart_with_items):
    ...
    def test_cart_assign_to_user(self, existing_user):
    ...
    def test_cart_total_price(self, create_cart_with_items):
        # Тест, что цена считается правильно
        cart = create_cart_with_items
        total_price = cart.get_total_price()

        expected_price = sum(
            item.price * item.quantity for item in cart.cartitem_set.all()
        )
        assert total_price == expected_price

    def test_cart_total_items(self, create_cart_with_items):
